...
class MyTodoController {
  ...
  createChallengedTodo = async (req, res) => {
  ...
  deleteChallengedTodo = async (req, res) => {
    const { userId } = res.locals.user;
  ...
  completeChallengedTodo = async (req, res) => {
    const { challengedTodoId } = await this.joi.parameterSchema.validateAsync(
      req.params
    );
    const { userId } = res.locals.user;
  ...
  getUserTodo = async (req, res) => {
    ...
    const data = await this.myTodoService.getUserTodo(userId, elseUserId);
